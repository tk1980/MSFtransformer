...
class TensorWriter:
    ...
    def update(self, meter, epoch, suffix='train'):
        if is_main_process():
            if suffix == 'train':
                self.writer.add_scalar('LearningRate', meter.lr.global_avg, epoch)
            self.writer.add_scalar('Loss/'+suffix, meter.loss.global_avg, epoch)
            self.writer.add_scalar('Acc1/'+suffix, meter.acc1.global_avg, epoch)
            self.writer.add_scalar('Acc5/'+suffix, meter.acc5.global_avg, epoch)
            self.writer.close()
